<template>
  <div id="webcam-panel">
    ...
    <v-btn depressed large @click="toggleLiveView"
      >Live Viewing {{ liveViewing ? "OFF" : "ON" }}</v-btn
    >
  </div>
</template>
...
export default {
  ...
  methods: {
    setupWebcam(ssl, rotateMode, dwtObj) {
      ...
      this.webcamSources = this.webcamObj.GetSourceList().map((e) => {
        return { text: e, val: e };
      });
    },
    updateWebcamOptions() {
    ...
    updateWebcamSetting() {
      ...
      if (this.liveViewing) {
        this.webcamObj.PlayVideo();
      }
    },
    capture() {
      if (this.liveViewing) {
        this.toggleLiveView(false);
      }
      this.webcamObj.CaptureImage(
        () => {},
        (errCode, errStr) => {
