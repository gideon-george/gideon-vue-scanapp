<template>
  <div id="scan-panel">
    ...
    <div class="flex">
      <!-- Show UI Control -->
      <v-switch v-model="showUI" label="Show UI"></v-switch>
      <!-- Auto Feeder Control -->
      <v-switch v-model="autoFeeder" label="Auto Feeder"></v-switch>
      <!-- Duplex Control -->
      <v-switch v-model="duplex" label="Duplex"></v-switch>
    </div>
    <!-- Scan button -->
...
export default {
  ...
  methods: {
    ...
    setupScan(dwtObj) {
      this.dwtObj = dwtObj;
      this.scanners = dwtObj.GetSourceNames().map((scanner, idx) => {
        return { id: idx, text: scanner };
      });
    },
  },
};
