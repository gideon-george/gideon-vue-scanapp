<template>
  <div id="viewer-container" :style="{ width: width, height: height }">
    ...
    <div id="edit-button-group">
      <v-btn outlined tile color="grey darken-1" small @click="rotate(-1)"
        >Rotate Left</v-btn
      >
      <v-btn outlined tile color="grey darken-1" small @click="rotate(1)"
        >Rotate Right</v-btn
      >
      <v-btn outlined tile color="grey darken-1" small @click="flip()"
        >Flip</v-btn
      >
      <v-btn outlined tile color="grey darken-1" small @click="mirror()"
        >Mirror</v-btn
      >
    ...
    mountViewer(dwtObj) {
      if (!dwtObj) {
        ...
        if (dwtObj.BindViewer(this.id, viewOptions)) {
          ...
          dwtObj.RegisterEvent("OnMouseClick", (idx) => {
            this.currIdx = idx;
          });
        }
      }
